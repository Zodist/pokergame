        ...
        public void run() {
            ...
            try {
                ...
                while (true) {
                    do {
                        string = br.readLine();
                        Log.d("InetSocket", "get data : " +string );
                    } while (string.length() == 0);

                    if (reqAck != null) {
                        Log.d("InetSocket","connecting is not done!"+string );
                        if (string.contains(resAck) == true) {
                            ringProgressDialog.dismiss();
                            reqAck = null;
                        }
                        else {
                            Log.d("InetSocket","sleep in Recv  : "+string );
                    ...
                    Message msg = Message.obtain();
                    msg.obj = string;
                    Log.d("InetSocket","tmp in Recv  : "+string );
